...
def test_route_kplus_notification():
    ...
    txn, err, is_slip = _parse_event(None, 1, "kplus", payload, FB)
...
def test_route_line_app_scb():
    ...
    txn, err, is_slip = _parse_event(None, 1, "line", payload, FB)
...
def test_route_uob_source():
    ...
    txn, err, is_slip = _parse_event(None, 1, "uob", payload, FB)
    assert txn is not None and txn.bank == "UOB"


def test_route_unknown_source():
    txn, err, is_slip = _parse_event(None, 1, "whatever", {}, FB)
...
def test_route_gallery_is_detected_as_slip():
    ...
    txn, err, is_slip = _parse_event(None, 1, "gallery-kplus", {"path": "x.jpg"}, FB)
